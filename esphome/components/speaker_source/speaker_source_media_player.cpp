...
void SpeakerSourceMediaPlayer::handle_media_state_changed_(uint8_t pipeline, media_source::MediaSource *source,
  ...
  if (state == media_source::MediaSourceState::IDLE) {
    ...
    if (pipeline == ANNOUNCEMENT_PIPELINE) {
      this->announcement_finish_hold_saw_announcement_ = true;
      if (this->announcement_finish_hold_active_) {
        this->announcement_finish_hold_pending_ms_ = this->announcement_finish_hold_duration_ms_;
        this->announcement_finish_hold_active_ = false;
      }
    }
...
void SpeakerSourceMediaPlayer::loop() {
  ...
  } else if ((announcement_playlist_active || this->hold_finished_announcement_()) &&
             old_state == media_player::MEDIA_PLAYER_STATE_ANNOUNCING) {
    this->state = media_player::MEDIA_PLAYER_STATE_ANNOUNCING;
  } else {
...
// THREAD CONTEXT: Called from main loop via YAML lambda automation.
void SpeakerSourceMediaPlayer::set_announcement_finish_hold_ms(uint32_t hold_ms) {
  if (hold_ms == 0) {
    this->announcement_finish_hold_pending_ms_ = 0;
    this->announcement_finish_hold_active_ = false;
    this->announcement_finish_hold_duration_ms_ = 0;
    this->announcement_finish_hold_saw_announcement_ = false;
    return;
  }

  if (this->announcement_finish_hold_active_) {
    this->announcement_finish_hold_pending_ms_ = 0;
    this->announcement_finish_hold_started_ms_ = millis();
    this->announcement_finish_hold_duration_ms_ = hold_ms;
  } else {
    this->announcement_finish_hold_pending_ms_ = hold_ms;
  }
}

bool SpeakerSourceMediaPlayer::hold_finished_announcement_() {
  PipelineContext &ann_ps = this->pipelines_[ANNOUNCEMENT_PIPELINE];

  bool announcement_drained = ann_ps.active_source.load(std::memory_order_relaxed) == nullptr &&
                              ann_ps.playlist_index >= ann_ps.playlist.size() &&
                              (ann_ps.repeat_mode == REPEAT_OFF || ann_ps.playlist.empty());
  if (!announcement_drained || !this->announcement_finish_hold_saw_announcement_) {
    return false;
  }

  if (!this->announcement_finish_hold_active_) {
    if (this->announcement_finish_hold_pending_ms_ == 0) {
      return false;
    }
    this->announcement_finish_hold_active_ = true;
    this->announcement_finish_hold_started_ms_ = millis();
    this->announcement_finish_hold_duration_ms_ = this->announcement_finish_hold_pending_ms_;
    this->announcement_finish_hold_pending_ms_ = 0;
    ESP_LOGD(TAG, "Holding finished announcement state for %u ms",
             static_cast<unsigned>(this->announcement_finish_hold_duration_ms_));
  }

  if (millis() - this->announcement_finish_hold_started_ms_ < this->announcement_finish_hold_duration_ms_) {
    return true;
  }
